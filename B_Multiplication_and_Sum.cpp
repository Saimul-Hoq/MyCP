...
void push(int i, int b, int e)
{
    ...
    tree[i] = ((lazy[i])*tree[i])%mod;
    if(b!=e)
    {
        int l = 2*i, r = l+1;
        lazy[l] = (lazy[l]*lazy[i])%mod;
        lazy[r] = (lazy[r]*lazy[i])%mod;
    }
    lazy[i] = 1;
}
...
ll segment_query(int i, int b, int e, int qb, int qe)
{
    ...
    int mid = b + (e-b)/2;
    int l = 2*i, r = 2*i+1;
    return (segment_query(l, b, mid, qb, qe) + segment_query(r, mid+1, e, qb, qe))%mod;
}
...
void saim()
{
    ...
    while(m--)
    {
        ...
        else
        {
            ...
            cin>>a>>b;
            a++;
            cout<<segment_query(1, 1, n, a, b)<<endl;
        }
    }
}
